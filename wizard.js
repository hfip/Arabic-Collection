...
(function () {
  ...
  const state = {
    ...
    token: null,
    profiles: [],
    selectedالملف الشخصيId: null,
    createNewالملف الشخصي: true,
    resultالملف الشخصيName: '',
    errorMsg: '',
  };
  ...
  function render() {
    ...
    if (state.step === 'profile') return renderالملف الشخصي(panel);
    if (state.step === 'pushing') return renderPushing(panel);
    if (state.step === 'done') return renderتم(panel);
    if (state.step === 'error') return renderError(panel);
  }

  function header(title, subtitle, withرجوع) {
    return `
      <div class="wiz-header">
        ${withرجوع ? `<button class="wiz-back" id="wiz-back" title="رجوع">${ICON.back}</button>` : ''}
  ...
  function renderالملف الشخصي(panel) {
    const opts = state.profiles.map((p) =>
      `<option value="${p.profile_index}" ${state.selectedالملف الشخصيId === p.profile_index && !state.createNewالملف الشخصي ? 'selected' : ''}>${escapeHtml(p.name)}</option>`
    ).join('');
      ...
      <div class="wiz-body">
        <label class="wiz-label">الملف الشخصي
          <select id="wiz-profile-select" class="wiz-input">
            <option value="__new__" ${state.createNewالملف الشخصي ? 'selected' : ''}>➕ إنشاء ملف شخصي جديد (موصى به)</option>
            ${opts}
          </select>
        </label>

        <div id="wiz-newprofile-wrap" class="wiz-label" style="${state.createNewالملف الشخصي ? '' : 'display:none;'}">
    ...
    select.addEventListener('change', () => {
      if (select.value === '__new__') {
        state.createNewالملف الشخصي = true;
        state.selectedالملف الشخصيId = null;
      } else {
        state.createNewالملف الشخصي = false;
        state.selectedالملف الشخصيId = Number(select.value);
      }
      el('wiz-newprofile-wrap').style.display = state.createNewالملف الشخصي ? '' : 'none';
  ...
  function profileNoteText() {
    return state.createNewالملف الشخصي
    ...
    panel.innerHTML = `
      ...
      <div class="wiz-body wiz-center">
        ...
        <p class="wiz-done-text">
          ...
          <strong>"${escapeHtml(state.resultالملف الشخصيName)}"</strong> .
  ...
  async function onAccountContinue() {
    ...
    try {
      if (state.mode === 'create') {
        ...
        return await pushToNewالملف الشخصي(state.profileName.trim() || DEFAULT_PROFILE_NAME);
      } else {
        state.pushingLabel = 'جاري تسجيل الدخول...';
        go('pushing');
        const auth = await window.NuvioPush.login(state.email, state.password);
        state.token = auth.token;
        state.profiles = await window.NuvioPush.getالملف الشخصيs(state.token);
        // Default to the safe "create new profile" choice.
        state.createNewالملف الشخصي = true;
        state.selectedالملف الشخصيId = state.profiles[0] ? state.profiles[0].profile_index : null;
        go('profile');
      }
    } catch (err) {
  ...
  async function onPush() {
    ...
    if (pn) state.profileName = pn.value;
    try {
      if (state.createNewالملف الشخصي) {
        await pushToNewالملف الشخصي(state.profileName.trim() || DEFAULT_PROFILE_NAME);
      } else {
        state.pushingLabel = 'جاري تحميل المجموعة...';
        go('pushing');
        const profile = state.profiles.find((p) => p.profile_index === state.selectedالملف الشخصيId);
        await doPush(state.selectedالملف الشخصيId, profile ? profile.name : `الملف الشخصي ${state.selectedالملف الشخصيId}`);
      }
    } catch (err) {
      state.errorMsg = (err && err.message) || String(err);
      go('error');
  ...
  async function pushToNewالملف الشخصي(name) {
    state.pushingLabel = 'جاري إنشاء الملف الشخصي...';
    go('pushing');
    const profile = await window.NuvioPush.createالملف الشخصي(state.token, name);
  ...
  async function doPush(profileId, profileName) {
    ...
    state.resultالملف الشخصيName = profileName;
    go('done');
  }
